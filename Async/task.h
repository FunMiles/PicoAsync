...
struct [[nodiscard]] task {
  ...
  explicit task(coroutines::coroutine_handle<promise_type> handle)
      : my_handle(handle)
  {
  }

  task(task &&t) noexcept
      : my_handle(t.my_handle)
  {
    t.my_handle = nullptr;
  }
  ...
  auto operator co_await() noexcept
  {
    struct awaiter {
      ...
      coroutines::coroutine_handle<> await_suspend(coroutines::coroutine_handle<> awaiting_coroutine) noexcept
      {
...
struct task<T, lazy>::task_promise : public promise_setter<T> {
  ...
  /// \brief Invoked when we first enter a coroutine, build the task object with the coroutine handle.
  ...
  auto final_suspend() const noexcept
  {
    struct awaiter {
      ...
      /// \details await_suspend will be called next allowing to return the handle of the continuation.
      bool await_ready() const noexcept { return false; }

      void await_resume() const noexcept {}
      /** \brief Suspension method for the ending co-routine.
			 * \details The returned handle will be resumed.  Either this co-routine is at a root of a complete task tree
			 * or it is a branch. In the first case, a noop handle is returned, otherwise the continuation handle is returned.
			 * It is possible to return noop and schedule the continuation back on the task system instead in order to implement
       ...
       */
      coroutines::coroutine_handle<> await_suspend(coroutines::coroutine_handle<task::task_promise> h) noexcept
      {
...
//    std::cerr << "Unhandled exception caught...\n";
  ...
  void set_continuation(coroutines::coroutine_handle<> handle)
  {
    continuation = handle;
  }
